...
public class ModuleStorageUI : MonoBehaviour {
    ...
    {
        //调整UI数量
        if (moduleUIList.Count < StorageManager.instance.GetModuleCount())
        {
            expandStorageUI();
        }
        ...
        for (int i=0;i< moduleUIList.Count; i++)
        {
            if (i < StorageManager.instance.GetModuleCount())
            {
                moduleUIList[i].GetComponent<DragAndDrop>().moduleType = StorageManager.instance.GetModule(i).type;
                moduleUIList[i].SetActive(true);
            }
            else
            {
                moduleUIList[i].GetComponent<DragAndDrop>().moduleType = ModuleType.Locked;
