  ...
  void InstantiateMap()
  {
    ...
    */
    ...
    for (int i = 0; i < 2; i ++)
    {
      for(int j = 0; j < 2; j ++)
      {
        map[1 + i, width_cnt / 2 - 1 + j] = 2;// + (int)GameManager.Direction.Down;
        map[height_cnt - 2 - i, width_cnt / 2 - 1 + j] = 2;// + (int)GameManager.Direction.Up;
        map[height_cnt / 2 - 1 + j, 1 + i] = 2;// + (int)GameManager.Direction.Left;
        map[height_cnt / 2 - 1 + j, width_cnt - 2 - i] = 2;// + (int)GameManager.Direction.Right;
      }
    }
